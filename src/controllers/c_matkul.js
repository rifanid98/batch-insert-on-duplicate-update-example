...
/** Model */
const eventsModel = require("../models/m_matkul");
...
async function getMatkul(req, res) {
	try {
		...
		// await validate.validateMatkul(req.body, fieldToPatch);
...
async function postMatkul(req, res) {
	try {
		// Joi validation
		const fieldToPatch = Object.keys(req.body);
		await validate.validateMatkul(req.body, fieldToPatch);
		...
		const body = req.body;
		const checkMatkul = await eventsModel.getDataByName(body.title);
		
		if (checkMatkul.length > 0) {
			if (req.file) {
