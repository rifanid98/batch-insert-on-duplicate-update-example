...
module.exports = {
	 ...
	 */
	validateTemplate: function (schema, fields, field = null) {
		/** Example of schema */
		const exampleSchema = {
		...
		if (!field) {
			...
			return new Promise((resolve, reject) => {
				const error = Joi.validate(fields, dynamicSchema);
	...
	validateScores: function(fields, field = null) {
		const schema = {
			mahasiswa_id: Joi.string().required(),
			matkul_id: Joi.string().required(),
			score: Joi.number().min(0).max(100).required(),
			description: Joi.string().min(3).required()
		};
		return this.validateTemplate(schema, fields, field);
	},
